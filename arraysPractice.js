...
//evens array (all the even values from nums) and the second item in the
//Array being the odds array(all the odd values from nums).

function divider(numbersArray) {
  var arr = [[],[]];
  for (var i = 0; i < numbersArray.length; i++) {
    if(numbersArray[i]%2===0)arr[0].push(numbersArray[i]);
    else arr[1].push(numbersArray[i]);
  }
  return arr;
}
...
//number (by invoking getRandomArbitrary), then loop through the array
// (that will be passed in as a parameter) to see if that random number
...
//Write a function called 'longer' that is given arr1 and arr2 as it's
// only arguments. Return the array which is longest.
...
function both(arr1,arr2) {
  ...
  for (var i = 0; i < arr1.length; i++) {
    for (var n = 0; n < arr2.length; n++) {
      if(arr1[i]===arr2[n])unity.push(arr2[n]);
    }
  }
  return unity;
}
...
devMountainEmployees.push(tyler);devMountainEmployees.push(cahlan);
devMountainEmployees.push(ryan);devMountainEmployees.push(colt);
...
users.push(user0);users.push(user1);
users.push(user2);users.push(user3);
//Your Code Here
...
Once you find the particular index he's located in, delete him from the array.*/
  ...
  for (var i = 0; i < users.length; i++) {
    var x = users[i].name.split(" ");
    if(x[0]==="Tyler")users.splice(i,1);
  }
